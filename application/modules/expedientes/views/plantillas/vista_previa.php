            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    ...
                    <div class="col-xs-6" style="border-right: 1px solid blue;">
                        <form id="form_plantilla" name="form_plantilla" method="post" action="<?=base_url('expedientes/plantillas/create_pdf')?>" onsubmit="//javascript:onDone();">
                            <input type="hidden" name="fecha" value="<?=  date('d-m-Y')?>">
                            <fieldset v-for="(form, index) in vforms" v-bind:id="'form_'+index" class="form-group" style="margin-top: 3em;">
                                ...
                                <div class="form-group" v-for="element in form.elements">
                                    <div v-if="element.type != 'submit'" >
                                        <label for="element.name">{{element.label}} {{ toBoolean(element.isrequired) ? "*" : "" }}</label>
                                        <input v-if="element.element == 'input'" v-on:keyup="actionInput(element)" v-bind:type="element.type" v-bind:class="element.class" v-bind:id="element.name" v-bind:name="element.name" v-model="element.value" v-bind:readonly="toBoolean(element.disable)" v-bind:required="toBoolean(element.isrequired)">
                                        <select v-if="element.element == 'select'" v-bind:class="element.class" v-bind:id="element.name" v-bind:name="element.name" v-html="element.options" v-model="element.value" v-bind:readonly="toBoolean(element.disable)" v-bind:required="toBoolean(element.isrequired)">

                                        </select>
                                        <textarea v-if="element.element === 'textarea'" v-bind:class="element.class" v-bind:id="element.class" v-bind:id="element.name" v-bind:name="element.name" v-model="element.value" v-bind:readonly="toBoolean(element.disable)" v-bind:required="toBoolean(element.isrequired)"></textarea>
                                    </div>
                                </div>
                            ...
                            <div id="form_pad_sign">
                                ...
                                <br>
                                <p>
                                    <textarea type="hidden" name="firmapad" id="firmapad" cols="5" class="form-control"></textarea>
                                    <br>
