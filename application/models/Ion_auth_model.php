...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function change_password($identity, $old, $new = NULL, $first_name = NULL, $last_name = NULL)
	{
		$this->trigger_events('pre_change_password');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($this->config->item('login_infogov'))
		{
			$query = $this->db->select('CodiUsua, CucuPers, InhaUsua')
				->where('CodiUsua', $identity)
				->where('ClavUsua', md5($password))
				->limit(1)
				->get("$this->sigmu_schema.usuario");

			if ($query->num_rows() === 1)
			{
				$user_infogov = $query->row();
				if ($user_infogov->InhaUsua == 1)
				{
				...
				if ($query_zetta->num_rows() === 0)
				{
					$ip_address = $this->_prepare_ip($this->input->ip_address());
					$password = $this->hash_password($password, FALSE);
					$this->db->insert('users', array(
						'CodiUsua' => $user_infogov->CodiUsua,
						'ip_address' => $ip_address,
						'username' => $user_infogov->CodiUsua,
						'password' => $password,
						'email' => '',
						'active' => 1,
						'created_on' => time(),
				...
				if (!empty($oficinas))
				{
					$user->oficina_actual = $oficinas[0]->nombre;
					$user->oficina_actual_id = $oficinas[0]->id;
                                        $user->inicia_expediente = $oficinas[0]->inicia_expediente;
				}
                                
                                $query_groups = $this->db->select('g.name')
                                        ->join('groups g', 'ug.group_id = g.id')
                                        ->where('ug.user_id', $user->id)
                                        ->get('users_groups ug');
                                $user->groups = $query_groups->result();
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'oficina_actual'       => isset($user->oficina_actual) ? $user->oficina_actual : '',
	...
	**/
	...
	public function desbloquear() {
		return $this->db->empty_table($this->tables['login_attempts']);
	}
}
