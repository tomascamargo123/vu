  ...
  class Datatables
  {
    ...
    */
    ...
    private $edit_columns   = array();
		private $exact_where_columns    = array();
    ...
    */
	 ...
	 * @return mixed
	 */
	public function exact_where_column($column)
	{
		$column = explode(',', $column);
		$this->exact_where_columns = array_merge($this->exact_where_columns, $column);
		return $this;
	}
    ...
    private function get_filtering()
		{
			...
			if ($sSearch != '')
			{
				for ($i = 0; $i < count($mColArray); $i++)
				{
					if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
						if ($this->check_cType())
							$sWhere_or[] = $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%'";
						else
							$sWhere_or[] = $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%'";
				}
			}
			...
			for ($i = 0; $i < count($mColArray); $i++)
			{
				if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns) && $mColArray[$i]['search']['value'] != '')
				{
					...
					if (in_array($mColArray[$i]['data'], $this->exact_where_columns))
					{
						if ($this->check_cType())
							$sWhere_and[] = $this->select[$mColArray[$i]['data']] . " = '" . $sSearch . "'";
						else
							$sWhere_and[] = $this->select[$this->columns[$i]] . " = '" . $sSearch . "'";
					} 
					else
					{
						if ($this->check_cType())
							$sWhere_and[] = $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%'";
						else
							$sWhere_and[] = $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%'";
					}
				}
			}

		if (!empty($sWhere_or) && !empty($sWhere_and))
			{
				$sWhere = '(' . join(' OR ', $sWhere_or) . ') AND ' . join(' AND ', $sWhere_and);
			}
			elseif (!empty($sWhere_and))
			{
				$sWhere = join(' AND ', $sWhere_and);
			}
			elseif (!empty($sWhere_or))
			{
				$sWhere = join(' OR ', $sWhere_or);
			}
    ...
    */
    private function produce_output($output, $charset)
    {
	  ...
	  else
	  {
		foreach ($rResult->result_array() as $row_key => $row_val)
		{
		  $aaData[$row_key] = array_values($row_val);
		  ...
		  foreach ($this->edit_columns as $modkey => $modval)
			...
			$aaData[$row_key] = array_diff_key($aaData[$row_key], array_intersect($this->columns, $this->unset_columns));
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      #Código agregado - No pertenece a la libreria real
      if($this->limit > 0)
      {
    ...
    */
    private function exec_replace($custom_val, $row_data)
    {
      ...
      if(preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches) && is_callable($matches[1]))
          ...
          $custom_val['content'] = str_ireplace('$' . ($key + 1), htmlentities($replace_string), $custom_val['content']);
        }
      }
    ...
    */
     ...
     * Agregado
     */
    public function set_limit($val = 0){
      $this->limit = $val;
    }
